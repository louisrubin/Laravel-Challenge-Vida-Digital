...
@section('h1')
    Viendo ({{ count($allProductos) }})  
    @if (count($allProductos) == 1)
        producto
    @else
        productos
    @endif   
                ...
                <div class="card">
                    ...
                    <table class="text-center mb-2">
                        <tr class="bg-dark text-white">
                            <th class="py-3">Cód. Barra</th>
                            <th>Nombre Prod.</th>
                            <th>Unid. por Bultos</th>
                            <th>Precio Bulto</th>
                            <th>Acción</th>
                        </tr>
                        ...
                        @foreach ($allProductos as $item)
                            <tr>
                                ...
                                <td class="text-capitalize">{{ $item->nombre_prod }}</td>
                                <td>{{ $item->unid_x_bulto }} <small>Un.</small></td>
                                <td>${{ $item->precio_vent_bulto }}</td>
                                <td>
                                    <a href="{{ route('mostrarProducto', ['codigo' => $item->cod_barra] ) }}" class="text-decoration-none me-1">
                                    ...
                                    {{-- DELETE ITEM FORM --}}
                                    <form method="POST" class="d-inline" action="{{ route('eliminarProducto', [$item->cod_barra])  }}">
                                        @csrf
                                        @method('DELETE')
